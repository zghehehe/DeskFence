...
Rust + Win32 + Direct2D 实现，单文件、免安装、绿色便携。
...
- **多显示器 + 高 DPI**；开机自启动（托盘开关），启动即现（内置图标缓存）
...
1. 到 [Releases](../../releases) 下载 `deskfence.exe`
2. 双击运行即可。首次运行如遇 SmartScreen 提示：点"更多信息 → 仍要运行"
3. 程序常驻托盘（右下角图标），右键托盘图标进行管理

单文件、静态链接，**无需安装任何运行库**（Windows 10/11 x64 系统库即够）。
...
```bash
git clone https://github.com/<你的用户名>/DeskFence.git
cd DeskFence
cargo build --release
...
- **开机后栅栏多久出现？** 进程拉起后约 0.3 秒全量呈现（内置持久化图标
  缓存，冷启动同样生效）。
