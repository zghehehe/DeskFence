//! DeskFence 库目标（2026-09-16 从 main.rs 拆出，纯搬家不改行为）：
//! 全部模块经 `pub mod` 暴露，供 tests/ 目录的集成测试访问
//! （tests/ 仅存在于私有开发仓，不随对外源码发布）。
//! 可执行入口仍是 src/main.rs（持 windows_subsystem 属性），本体在这里。

// 硬门禁：unsafe fn 体内的 unsafe 操作必须显式包 unsafe {} 块，
// 不再依赖函数级 unsafe 上下文的隐式传播（2026-09-17 起）。
#![deny(unsafe_op_in_unsafe_fn)]

// ---- 目录分组(2026-09-17 起五组分类)----
// foundation/ 纯逻辑与状态基础 · platform/ Win32 系统互操作 · gfx/ 渲染
// interact/ 用户交互 · runtime/ 运行期呈现与守护 · 顶层 ui=编排壳。
// 分组只改物理位置;旧扁平路径 crate::<模块> 经下方 pub use 重导出
// 全部保留,调用点与集成测试零改动。
pub mod foundation {
    pub mod lang;
    pub mod logging;
...
pub use foundation::{lang, logging, model, settings, state};
pub use gfx::{iconcache, render};
pub use interact::{cats_panel, drag, firstrun, menu, ole, rename};
pub use platform::{hosts, metrics, monitors, shell, wallpaper, winids};
pub use runtime::{envhealth, present, selfheal, undo};
...
fn ensure_clean_startup() {
    ...
    logging::log("clean-start: desktop host not ready, waiting (max 15s)");
    for _ in 0..30 {
        std::thread::sleep(std::time::Duration::from_millis(500));
        if ui::desktop_host_ready() {
            logging::log("clean-start: desktop host became ready");
            return;
        }
    }
    logging::log("clean-start: host still absent after 15s, booting with deferred attach");
}

/// 清场:让旧实例退出,先礼后兵——先给旧实例托盘窗发 WM_CLOSE 走 quit_app 的
...
fn clear_previous_instances() {
    ...
    unsafe {
        let tray = FindWindowW(winids::tray_class_name(), PCWSTR::null()).unwrap_or_default();
        if !tray.0.is_null() {
            let _ = PostMessageW(Some(tray), WM_CLOSE, WPARAM(0), LPARAM(0));
        }
    }
    for _ in 0..20 {
