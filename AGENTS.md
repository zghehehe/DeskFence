...
6. **代码卫生即做即清（2026-09-09 用户明确要求，长期有效）**：每次修改或
   ...
   - **零警告水位**：动过代码后 `cargo build --release` 与
     ...
     门禁，2026-09-09 起），测试全绿再提交。**CI 红而本地绿 → 先怀疑
     工具链版本差**：runner 的 `@stable` 通常比本地新（2026-09-11 实锤：
     1.98 新增 lint `chunks_exact_to_as_chunks`，BGRA 缓冲的
     `chunks_exact(4)` 四处红灯而本地 1.97 全绿；修=迁 `as_chunks`，
     1.88+ 稳定两端兼容）。取证办法：ci.yml 临时加"失败时 tee 落盘并
     推回 ci-log 分支"，SSH 拉回即读（私仓无 API 凭证时的自力取证法，
     用完即撤）。
