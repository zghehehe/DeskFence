...
6. **菜单前台权与闪屏(2026-08-25 修复，勿回退）**：所有 TrackPopupMenu 调用前必须
   ...
   - **"点桌面关菜单闪屏"是四重根因叠加**，全修于 2026-08-25：
     ...
     **③ 的判定本身 2026-08-27 又挖出致命 bug（"刚开始不闪、菜单操作后
     ...
     每秒出现即此 bug 复发。
     **④ 2026-08-27 终修（"点倒三角/托盘后再点桌面闪"）**：真点击实测
     （SPES 当日已放开合成点击）复现出每次菜单交互都触发全量 SetWindowPos
     =可见闪。三个触发源：用户最小化的窗口沉到 Progman 与栅栏包之间、
     物理点击把被点栅栏瞬时升 z、自动隐藏任务栏在弹出/隐藏转换时短暂沉
     入桌面层——旧的"单趟收集式"判定撞上任何一个就判全列失位。终版判定
     （**勿回退**）：**逐栅栏**从其宿主向上（GW_HWNDPREV）走到命中该栅栏
     为止；途中跳过一切不可见窗（IsIconic/隐藏/矩形与虚拟屏幕不相交——
     含最小化沉底、SPES 离屏 CoreWindow）与自有辅助窗（菜单宿主/托盘窗/
     #32768/Shell_TrayWnd——任务栏与栅栏矩形天然不相交）；只有**可见外
     来窗口与该栅栏矩形相交**才把该栅栏单独移回（只动这一个）。修复日志
     带 occluder 类名。测试要点：托盘菜单从图标**向上**展开，空白点击点
     必须先量菜单矩形再选（否则点到菜单第一项=误触发"隐藏全部栅栏"）。
     验收基线（tools/matrix2.ps1）：tray/triangle × 重启后 1s/10s/30s/
     60s/120s 共 10 相，全部"菜单实测开→实测关+栅栏数不变+0 repairs+
     闪事件仅菜单自身像素"。
