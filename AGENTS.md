...
11. **z 序守卫体系（2026-08-28 终修，勿回退）**：两个用户可见症状
   ...
   - **修复三层**（ui.rs，全部可独立回退）：① `ZIntent` 线程局部意图
     ...
     太热不采用。
   - **桌面态快速轮询 + 粘底（2026-08-28 终版，勿回退）**：三指手势的
     ...
     动作时消耗**。配套"粘底"：走查发现栅栏垫在死层（一路只有可忽略窗）
     且宿主正上方非自家栅栏时归位到带底（z-glue 日志）——恢复批次把
     ~320 个窗插回低位时栅栏必然被顶高（无论隐藏时在哪），粘底保证稳态
     落位正确。bandtest 的 phaseB 断言已随双通道语义更新
