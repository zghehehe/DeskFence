...
6. **菜单前台权与闪屏(2026-08-25 修复，勿回退）**：所有 TrackPopupMenu 调用前必须
   ...
   - **"点桌面关菜单闪屏"是四重根因叠加**，全修于 2026-08-25：
     ...
     **③ 的判定本身 2026-08-27 又挖出致命 bug（"刚开始不闪、菜单操作后
     ...
     每秒出现即此 bug 复发。
     **④ 2026-08-27 二次补刀（"点倒三角/托盘后再点桌面偶发闪"终修）**：
     真点击实测（SPES 当日已放开合成点击）复现出每次菜单交互 2 次修复。
     两个触发源：①用户最小化任意窗口会沉到 Progman 与栅栏包之间；②物理
     点击会把被点栅栏瞬时升到最小化层之上——遍历撞上这些**不可见**窗口
     即失配→修复→闪。修：遍历跳过"不可见"窗口（IsIconic ∨ !IsWindowVisible
     ∨ 矩形与虚拟屏幕不相交——离屏 CoreWindow/#32770 一并覆盖），且修复
     改为**按需**（只动没被遍历到的栅栏，已就位的一个不碰）。修复日志带
     culprit 窗口类名。验收基线：tools/confirm.ps1 两态（干净/MinimizeAll
     沉底+点击）均 0 repairs、0 闪事件；tools/matrix.ps1 重启+10s/60s 同。
     **勿回退**。
