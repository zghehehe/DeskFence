...
2. **疑虑/冲突主动确认**：发现用户新指令与 AGENTS.md 既有约定、历史决策或
   日志事实有冲突、矛盾或含糊时，**主动向用户确认清楚再执行**，不要自行
   猜测静默取舍。实例：AGENTS.md 写"docs/ 不发布"，但 README 引用
   docs/demo.svg（实际 main 上有它）——这类矛盾应先问，不要自己推断。
